...
Shader "Indiez/Shader/Style_Indi_Skin"
{
	Properties
	{
		[TCP2HeaderHelp(Base)]
        ...
        _DissolveGradientTex ("Dissolve Gradient Texture", 2D) = "white" {}
        _DissolveGradientWidth ("Gradient Width", Range(0,1)) = 0.2
        _DissolveGradientStrength ("Gradient Strength", Range(0,5)) = 2
        [TCP2Separator]
	...
	SubShader
	{
        ...
        sampler2D _DissolveGradientTex;
		...
		CBUFFER_START(UnityPerMaterial)
            ...
            float _DissolveGradientStrength;
		CBUFFER_END
		...
		Pass
		{
			...
			half4 Fragment(Varyings input) : SV_Target
			{
                ...
                // --- DISSOLVE EFFECT ---
                float dissolveMap = tex2D(_DissolveMap, input.pack0.xy).r;
                float dissolveValue = _DissolveValue;
                float gradientWidth = _DissolveGradientWidth;
                float dissValue = dissolveValue * (1 + 2 * gradientWidth) - gradientWidth;

                // smooth dissolve transition (0..1)
                ...
                // sample gradient texture (use dissolveUV as U)
                float4 dissolveColor = tex2D(_DissolveGradientTex, float2(dissolveUV, 0));
                dissolveColor *= _DissolveGradientStrength * dissolveUV;

                // add emission glow to edges
