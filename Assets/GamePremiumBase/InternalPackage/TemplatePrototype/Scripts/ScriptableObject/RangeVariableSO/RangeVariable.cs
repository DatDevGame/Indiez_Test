...
public class RangeProgress<T>
{
    ...
    public void RecalculateInverseLerpValue(bool notifyDatasetChanged = true)
    {
        m_InverseLerpValue = m_RangeValue.CalcInverseLerpValue(value);
...
public abstract class RangeValue<T> : ISerializationCallbackReceiver
{
    ...
    [SerializeField, OnValueChanged("OnValidate")]
    ...
    public static implicit operator Range<T>(RangeValue<T> range)
    {
        return new Range<T>(range.minValue, range.maxValue);
    }
}
