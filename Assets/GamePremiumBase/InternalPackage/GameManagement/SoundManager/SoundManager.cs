...
public class SoundManager : Singleton<SoundManager>
{
    ...
    [SerializeField, Tooltip("Individual SFX settings preference")]
    PPrefBoolVariable m_SFXSound_PPref;
    [SerializeField, Tooltip("Individual BGM settings preference")]
    ...
    {
        base.Awake();
        m_Sound_PPref.onValueChanged += OnGlobalSoundPrefValueChanged;
    ...
    private void OnGlobalSoundPrefValueChanged(ValueDataChanged<bool> obj)
    {
        if (!m_Sound_PPref.value)
        {
            m_BGMAudioSource.Stop();
            m_SFXAudioSource.Stop();
            foreach (var source in m_LoopSource)
            {
                source.Value.Stop();
            }
        }
    ...
    /// <summary>
    /// Play one-shot SFX with given audio clip
    /// </summary>
    ...
    public void PlayLoopSFX(Enum SFXEnumKey, float volumn = 1f, bool loop = false, bool is3DSound = false, GameObject ownedGameObject = null)
    {
        ...
        else
        {
            ...
            if (m_LoopSource.ContainsKey(ownedGameObject))
            {
                ...
                loopAudioSource.enabled = m_SFXAudioSource.enabled;
                string sfxKey = String.Format("{0}.{1}", SFXEnumKey.GetType().Name, SFXEnumKey.ToString());
                if (m_GeneralSFXLibrarySO.Library.ContainsKey(sfxKey))
                {
                    var clip = m_GeneralSFXLibrarySO.Library[sfxKey];
    ...
    /// <summary>
    /// Set the current playing BGM volume
    /// </summary>
    ...
    /// <summary>
    /// Set the specific SFX looping volume
    /// </summary>
    /// <param name="ownedGameObject">owned game object of SFX</param>
    /// <param name="volume">volume value to set</param>
    public void SetSpecificLoopSFXVolume(GameObject ownedGameObject, float volume)
    {
        if (ownedGameObject == null) return;
        if (!m_LoopSource.ContainsKey(ownedGameObject)) return;
        m_LoopSource[ownedGameObject].volume = volume;
    }

    /// <summary>
    /// Set the specific SFX looping volume
    /// **Beware: this might cause multiple game objects containing the same clip to be affected too**
    /// </summary>
    /// <param name="SFXKey">SFX key of the SFX</param>
    /// <param name="volume">volume value to set</param>
    public void SetSpecificLoopSFXVolume(Enum SFXKey, float volume)
    {
        string SFXKeyStr = String.Format("{0}.{1}", SFXKey.GetType().Name, SFXKey.ToString());
        if (m_GeneralSFXLibrarySO != null && m_GeneralSFXLibrarySO.Library.ContainsKey(SFXKeyStr))
        {
            var clip = m_GeneralSFXLibrarySO.Library[SFXKeyStr];
            foreach (var i in m_LoopSource)
            {
                if (i.Value == clip)
                {
                    i.Key.GetComponent<AudioSource>().volume = volume;
                }
            }
        }
        if (m_SceneSFXLibrarySO != null && m_SceneSFXLibrarySO.Count > 0)
        {
            foreach (var i in m_SceneSFXLibrarySO)
            {
                if (i == null || i.Library == null || i.Library.Count <= 0 || !i.Library.ContainsKey(SFXKeyStr)) continue;
                var clip = i.Library[SFXKeyStr];
                foreach (var loopSource in m_LoopSource)
                {
                    if (loopSource.Value == clip)
                    {
                        loopSource.Key.GetComponent<AudioSource>().volume = volume;
                    }
                }
            }
        }
    ...
    /// <summary>
    /// Set the current playing SFX only, it doesn't include looped SFX
    /// </summary>
    /// <param name="volume">volume value to set</param>
