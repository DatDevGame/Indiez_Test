    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float fillMask = i.uv.x < fill;

                float3 texCol = tex2D( _MainTex, float2( fill, i.uv.y ) );
                float tFillColor = saturate( InverseLerp ( _LowHealthThreshold, _HighHealthThreshold, fill ) );
                float3 col = lerp( _LowHealthColor, _HighHealthColor, tFillColor );
                col *= texCol;

                if ( ( _PulseIfLow == 1 ) && ( fill <= 0.2 ) ) {
